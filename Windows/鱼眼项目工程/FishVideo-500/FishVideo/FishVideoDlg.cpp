...
BOOL CFishVideoDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_Phi0,_T("140"));
	SetDlgItemText(IDC_Sita0,_T("50"));
	SetDlgItemText(IDC_Gama0,_T("95"));

	SetDlgItemText(IDC_Phi1,_T("-85"));
	SetDlgItemText(IDC_Sita1,_T("95"));
	SetDlgItemText(IDC_Gama1,_T("95"));

	SetDlgItemText(IDC_Phi2,_T("270"));
	SetDlgItemText(IDC_Sita2,_T("45"));
	SetDlgItemText(IDC_Gama2,_T("85"));
...
//	myTrace.WriteToLog(" opv ");
	...
	Mat frame;
	//VideoWriter writer("C:\\Users\\Administrator\\Desktop\\testImage\\VideoTest.avi", CV_FOURCC('M', 'J', 'P', 'G'), 25.0, Size(640, 480));  
	ScanningMethod SM;
	//FisheyeImgCorrect FIC;
//	FIC.FisheyeImgCorrectInit();
	int flag = 0;
	...
	for(int i = 0;i < pDlg->frameNumber;++i)
	{
		...
		FIC.ImageCorrect(tempImg,dstImg1,dstImg2,dstImg3);
