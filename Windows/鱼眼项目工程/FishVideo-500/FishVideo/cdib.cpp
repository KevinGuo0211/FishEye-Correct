...
//	str.Format("i:%d, r:%d, g:%d, b:%d",i,pDibQuad->rgbRed,pDibQuad->rgbGreen,pDibQuad->rgbBlue);
//		AfxMessageBox(str);
...
BOOL CDib::Compress(CDC* pDC, BOOL bCompress /* = TRUE */)
{
	...
	("Compress: original palette size = %d\n", m_nColorTableEntries); 
	HDC hdc = pDC->GetSafeHdc();
...
BOOL CDib::Read(CFile* pFile)
{
	...
	// can't use bfSize in file header
 	...
 	BITMAPFILEHEADER bmfh;
 	try {
 		nCount = pFile->Read((LPVOID) &bmfh, sizeof(BITMAPFILEHEADER));//nCout=14

 		if(nCount != sizeof(BITMAPFILEHEADER)) {
			throw new CFileException;
 		}
	
 		if(bmfh.bfType != 0x4d42) {      //判断是否为位图文件  0x4D:M,0x42:B 
 			throw new CFileException;
 		}
 		nSize = bmfh.bfOffBits - sizeof(BITMAPFILEHEADER);//图像数据距文件头偏移量。
 		m_lpBMIH = (LPBITMAPINFOHEADER) new char[nSize];
 		m_nBmihAlloc = m_nImageAlloc = crtAlloc;
 		nCount = pFile->Read(m_lpBMIH, nSize); // info hdr & color table
		ComputeMetrics();
 	 	ComputePaletteSize(m_lpBMIH->biBitCount);         //设置调色板大小。。16位及其以上无调色板
 	    MakePalette();
 		m_lpImage = (LPBYTE) new char[m_dwSizeImage];  //m_dwSizeImage为整个文件大小
 		nCount = pFile->Read(m_lpImage, m_dwSizeImage); // image only即图像像素数据区大小
 	}
 	catch(CException* pe) {
...
BOOL CDib::ReadSection(CFile* pFile, CDC* pDC /* = NULL */)
{
	...
	try {
		...
		if(bmfh.bfType != 0x4d42) {          //0x4D:M,0x42:B 
			throw new CFileException;
		}
...
WORD CDib::GetSizePerLine()
{
	WORD wBytes = ((WORD) m_lpBMIH->biWidth * m_lpBMIH->biBitCount) / 32;
	if(((WORD) m_lpBMIH->biWidth * m_lpBMIH->biBitCount) % 32) 
	{
		wBytes++;
	}
	wBytes *= 4;
...
void CDib::Replacedib(CSize size, int nBitCount,BYTE *data)
{
	Empty();
	ComputePaletteSize(nBitCount);
	m_lpBMIH = (LPBITMAPINFOHEADER) new 
		char[sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * 256];
	m_nBmihAlloc = crtAlloc;
	m_lpBMIH->biSize = sizeof(BITMAPINFOHEADER);
	m_lpBMIH->biWidth = size.cx;
	m_lpBMIH->biHeight = size.cy;
	...
	{
		LPRGBQUAD tiaoseban=new RGBQUAD[256];
		for(int i=0; i<256;i++)
		{
			tiaoseban[i].rgbBlue=(BYTE)i; 
			tiaoseban[i].rgbGreen=(BYTE)i;   
			tiaoseban[i].rgbRed=(BYTE)i; 		 		 		
			tiaoseban[i].rgbReserved=0;		   	
		}		 	
		memcpy(m_lpvColorTable,tiaoseban, sizeof(RGBQUAD) *256);
		delete []tiaoseban;
	}
	m_lpImage = data;  
} 
